...
// Dev-only proxy for /api/grok so `npm run dev` works without `vercel dev`.
const devGrokProxy = {
  name: "dev-grok-proxy",
  configureServer(server: any) {
    server.middlewares.use("/api/grok", async (req: any, res: any, next: any) => {
      ...
      try {
        ...
        if (!apiKey) {
          ...
          res.end(JSON.stringify({ error: "Missing XAI_API_KEY" })); return;
        }

        const model = payload?.model || "grok-2-mini";
...
export default defineConfig(({ mode }) => {
  ...
  if (!process.env.XAI_API_KEY) {
    process.env.XAI_API_KEY = env.XAI_API_KEY || env.VITE_XAI_API_KEY || process.env.VITE_XAI_API_KEY;
  }
  return {
  plugins: [
    react(),
